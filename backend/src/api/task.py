...
@router.get(
    "/", response_model=list[SingleTask], dependencies=[Depends(JWTBearerService())]
)
async def get_tasks(
    task_service: TaskService = Depends(TaskService),
    user: models.User = Depends(JWTBearerService().get_current_user),
...
@router.put("/{task_id}", response_model=SingleTask, dependencies=[Depends(JWTBearerService())])
async def update_task(
    task_id: uuid.UUID, task: UpdateTask, task_service: TaskService = Depends(TaskService)
):
    try:
